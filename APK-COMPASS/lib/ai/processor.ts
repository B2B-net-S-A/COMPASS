...
const CANDIDATE_PROMPT = `Analyze the following text (CV, bio, or profile) and extract data in JSON format.
...
const PROJECT_PROMPT = `Analyze the following project specification and extract data in JSON format.
CRITICAL INSTRUCTIONS:
...
export async function processDataWithAI(text: string, type: ExtractionType): Promise<Record<string, unknown>> {
    ...
    const instructions = type === 'candidate' ? CANDIDATE_PROMPT : PROJECT_PROMPT

    return await chatJSON<Record<string, unknown>>({
        model: 'gpt-4o',
        system: systemPrompt,
