...
import { RatingWidget } from '@/components/akademia/RatingWidget'
...
export default async function QuizResultsPage({ params, searchParams }: PageProps) {
    ...
    return (
        <div className="p-6 md:p-8 max-w-3xl mx-auto space-y-6">
            <Link href={`/akademia/${course.slug}`} className="text-sm text-muted-foreground hover:text-primary inline-flex items-center gap-1">
                ← {course.title}
            </Link>
            ...
            >
                <CardContent className="p-8 text-center space-y-4">
                    ...
                    <div className="flex justify-center gap-2 pt-2">
                        {!passed && (
                            <Link href={`/akademia/${course.slug}/quiz`}>
                                <Button className="gap-2">
                                    <RefreshCw className="w-4 h-4" /> Spróbuj ponownie
                                </Button>
                            </Link>
                        )}
                        <Link href={`/akademia/${course.slug}`}>
