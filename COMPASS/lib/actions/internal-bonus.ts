...
// `requireInvoicesEnabled()` rzuca zwykły Error, ale wyłączony moduł faktur to
// reguła biznesowa, nie awaria — bez konwersji użytkownik zobaczyłby komunikat
// ogólny, a Sentry dostawałoby zdarzenie przy każdym kliknięciu. Sam
// lib/feature-flags.ts zostaje nietknięty (poza zakresem tej migracji).
function requireInvoicesEnabledExpected(): void {
    try {
        requireInvoicesEnabled()
    } catch (err) {
        throw new ExpectedError(
            err instanceof Error ? err.message : 'Faktury są aktualnie wyłączone.',
        )
    }
}
...
function validateAmount(amount: number): void {
    if (!Number.isFinite(amount) || amount < BONUS_MIN_AMOUNT) {
        throw new ExpectedError(`Kwota musi być >= ${BONUS_MIN_AMOUNT}.`)
    }
    if (amount > BONUS_MAX_AMOUNT) {
        throw new ExpectedError(`Kwota za duża (max ${BONUS_MAX_AMOUNT}).`)
    }
}
...
function validateReason(reason: string): string {
    ...
    if (trimmed.length < BONUS_REASON_MIN_LENGTH) {
        throw new ExpectedError(`Uzasadnienie musi mieć co najmniej ${BONUS_REASON_MIN_LENGTH} znaki.`)
    }
    if (trimmed.length > BONUS_REASON_MAX_LENGTH) {
        throw new ExpectedError(`Uzasadnienie za długie (max ${BONUS_REASON_MAX_LENGTH} znaków).`)
    }
    return trimmed
}
...
function validateCurrency(currency?: string): void {
    if (currency && !/^[A-Z]{3}$/.test(currency)) {
        throw new ExpectedError('Waluta musi być w formacie ISO (np. PLN, EUR).')
    }
}

function validateProposeInput(input: ProposeBonusInput): void {
    if (!input.recipient_user_id) throw new ExpectedError('Pracownik jest wymagany.')
...
function validatePeriod(year: number, month: number): void {
    if (!Number.isInteger(year) || year < 2020 || year > 2100) {
        throw new ExpectedError('Niepoprawny rok.')
    }
    if (!Number.isInteger(month) || month < 1 || month > 12) {
        throw new ExpectedError('Niepoprawny miesiąc (1-12).')
    }
    const now = new Date()
...
function validateChampionsLeagueInput(year: number, quarter: number, placeRank: number): void {
    ...
    if (!Number.isInteger(quarter) || quarter < 1 || quarter > 4) {
        throw new ExpectedError('Niepoprawny kwartał (1-4).')
    }
    if (!Number.isInteger(placeRank) || placeRank < 1 || placeRank > 3) {
        throw new ExpectedError('Niepoprawne miejsce (1, 2 lub 3).')
    }
    if (!isQuarterInAllowedRange(year, quarter as 1 | 2 | 3 | 4)) {
        throw new ExpectedError('Kwartał musi mieścić się w ostatnich 4 kwartałach + bieżący.')
    }
}
...
function validateClientName(name: string | undefined | null): void {
    ...
    if (client.length < 2) {
        throw new ExpectedError('Klient: minimum 2 znaki.')
    }
}
...
function validateCategoryFields(input: AssignBonusInput): void {
    switch (input.category) {
        case 'sales': {
            ...
            if (desc.length < 3) {
                throw new ExpectedError('Opis usługi: minimum 3 znaki.')
            }
            break
        }
        case 'delivery_lead': {
            ...
            if (candidate.length < 3) {
                throw new ExpectedError('Imię i nazwisko kandydata: minimum 3 znaki.')
            }
            if (!Number.isFinite(input.delivery_margin_amount) || input.delivery_margin_amount <= 0) {
                throw new ExpectedError('Marża miesięczna musi być > 0.')
            }
            if (input.delivery_margin_percent !== undefined) {
                if (
                    ...
                    throw new ExpectedError('Procent premii musi być w zakresie 0-100.')
                }
            }
            break
        }
        case 'recruiter': {
            ...
            if (
                ...
                throw new ExpectedError('Marża rekrutera (PLN/h) musi być >= 0.')
            }
        ...
        case 'custom': {
            ...
            if (memo.length < 1) {
                ...
                throw new ExpectedError('Memo opisujące premię niestandardową jest wymagane (możesz też dodać załącznik po zapisaniu).')
            }
            if (memo.length > BONUS_CUSTOM_MEMO_MAX_LENGTH) {
                throw new ExpectedError(`Memo za długie (max ${BONUS_CUSTOM_MEMO_MAX_LENGTH} znaków).`)
            }
            break
        }
...
export async function linkBonusToInvoice(input: LinkBonusInput): Promise<ActionResult<BonusRow>> {
    return runAction('linkBonusToInvoice', async () => {
        requireInvoicesEnabledExpected()
        const ctx = await requireInternalOrAdminAction()
        if (!input.id || !input.invoice_id) {
            throw new ExpectedError('Brak id premii lub id faktury.')
        }
        ...
        const { data: bonus, error: fetchErr } = await supabase
            ...
            .eq('id', input.id)
            .single<BonusRow>()
        if (fetchErr || !bonus) throw new ExpectedError('Premia nie znaleziona.')
        if (bonus.recipient_user_id !== ctx.userId) {
            throw new ExpectedError('Możesz linkować tylko swoje premie.')
        }
        if (bonus.status !== 'pending') {
            throw new ExpectedError(`Premia w statusie "${bonus.status}" — link tylko dla "pending".`)
        }
        ...
        if (invErr || !invoice) throw new ExpectedError('Faktura nie znaleziona.')
        if (invoice.user_id !== ctx.userId) throw new ExpectedError('To nie Twoja faktura.')
...
/** @deprecated Phase 26 — invoice link path disabled. Gated by INVOICES_ENABLED. */
export async function unlinkBonus(id: string): Promise<ActionResult<BonusRow>> {
    return runAction('unlinkBonus', async () => {
        requireInvoicesEnabledExpected()
        const ctx = await requireInternalOrAdminAction()
        const supabase = createClient()
        ...
        if (bonus.recipient_user_id !== ctx.userId && !ctx.isAdmin) {
            throw new ExpectedError('Brak uprawnień do odlinkowania tej premii.')
        }
        if (bonus.status !== 'paid') {
            throw new ExpectedError(`Można odlinkować tylko premię w statusie "paid" (jest: "${bonus.status}").`)
        }
...
export async function proposeBonus(input: ProposeBonusInput): Promise<ActionResult<BonusRow>> {
    return runAction('proposeBonus', async () => {
        requireInvoicesEnabledExpected()
        const ctx = await requireBonusProposerAction()
        validateProposeInput(input)
        if (input.recipient_user_id === ctx.userId) {
            throw new ExpectedError('Nie można proponować premii dla siebie.')
        }
        ...
        if (!ctx.isAdmin) {
            ...
            if (!recipient) throw new ExpectedError('Odbiorca premii nie znaleziony.')
            if (recipient.manager_id !== ctx.userId) {
                throw new ExpectedError('Możesz proponować premie tylko dla swoich podwładnych.')
            }
        }
...
export async function cancelBonus(input: CancelBonusInput): Promise<ActionResult<BonusRow>> {
    return runAction('cancelBonus', async () => {
        ...
        if (!ctx.isAdmin && ctx.role !== 'finanse' && !ctx.isManager) {
            throw new ExpectedError('Wymagane uprawnienia: administrator, finanse lub manager.')
        }
        if (!input.id) throw new ExpectedError('Brak id premii.')
        const cancellationReason = (input.cancellation_reason ?? '').trim()
        if (cancellationReason.length < 3) {
            throw new ExpectedError('Powód anulowania musi mieć co najmniej 3 znaki.')
        }
        if (cancellationReason.length > 500) {
            throw new ExpectedError('Powód anulowania za długi (max 500 znaków).')
        }
        ...
        if (!ctx.isAdmin && ctx.role !== 'finanse' && bonus.proposed_by !== ctx.userId) {
            throw new ExpectedError('Manager może anulować tylko premie, które sam przypisał.')
        }
 ...
 */
export async function assignBonus(input: AssignBonusInput): Promise<ActionResult<BonusRow>> {
    return runAction('assignBonus', async () => {
        const ctx = await requireBonusProposerAction()
        validateAssignInput(input)
        if (input.recipient_user_id === ctx.userId) {
            throw new ExpectedError('Nie możesz przypisać premii samemu sobie.')
        }
        ...
        if (!ctx.isAdmin) {
            ...
            if (!recipient) throw new ExpectedError('Odbiorca premii nie znaleziony.')
            if (recipient.manager_id !== ctx.userId) {
                throw new ExpectedError('Możesz przypisać premię tylko swoim bezpośrednim podwładnym.')
            }
        }
 ...
 */
export async function updateBonus(input: UpdateBonusInput): Promise<ActionResult<BonusRow>> {
    return runAction('updateBonus', async () => {
        ...
        if (hasPeriod && (input.period_year === undefined || input.period_month === undefined)) {
            throw new ExpectedError('Aby zmienić miesiąc premii, podaj rok i miesiąc.')
        }
        if (!hasAmount && !hasReason && !hasNotes && !hasPeriod) {
            throw new ExpectedError('Brak zmian do zapisania.')
        }
        ...
        if (!ctx.isAdmin && ctx.role !== 'finanse' && bonus.proposed_by !== ctx.userId) {
            throw new ExpectedError('Manager może edytować tylko premie, które sam przypisał.')
        }
...
export async function listTeamBonuses(filter?: BonusListFilter): Promise<ActionResult<BonusWithUsers[]>> {
    return runAction('listTeamBonuses', async () => {
        const ctx = await requireInternalOrAdminAction()
        // RLS handles team scoping (manager sees own team, finanse/admin see all).
        if (!ctx.isAdmin && !ctx.isManager && ctx.role !== 'finanse') {
            throw new ExpectedError('Brak uprawnień do podglądu premii zespołu.')
        }
...
export async function uploadBonusAttachment(
    bonusId: string,
    file: File,
): Promise<ActionResult<{ path: string; filename: string; size: number; mime: string }>> {
    return runAction('uploadBonusAttachment', async () => {
        const ctx = await requireInternalOrAdminAction()
        if (!bonusId) throw new ExpectedError('Brak id premii.')
        if (!file || !(file instanceof File)) throw new ExpectedError('Brak pliku.')

        // Validate file
        if (file.size === 0) throw new ExpectedError('Plik jest pusty.')
        if (file.size > BONUS_ATTACHMENT_MAX_BYTES) {
            throw new ExpectedError(`Plik za duży (max ${BONUS_ATTACHMENT_MAX_BYTES / 1024 / 1024} MB).`)
        }
        ...
        if (bonus.proposed_by !== ctx.userId && !ctx.isAdmin) {
            throw new ExpectedError('Tylko proposer lub administrator może dodać załącznik.')
        }
 ...
 */
export async function getBonusAttachmentSignedUrl(bonusId: string): Promise<ActionResult<string>> {
    return runAction('getBonusAttachmentSignedUrl', async () => {
        const ctx = await requireInternalOrAdminAction()
        if (!bonusId) throw new ExpectedError('Brak id premii.')
        ...
        if (fetchErr || !bonus) throw new ExpectedError('Premia nie znaleziona.')
        if (!bonus.attachment_path) throw new ExpectedError('Ta premia nie ma załącznika.')
        ...
        if (!isRecipient && !isProposer && !ctx.isAdmin && !isFinance && !isManagerOfRecipient) {
            throw new ExpectedError('Brak uprawnień do pobrania załącznika.')
        }
 ...
 */
export async function removeBonusAttachment(bonusId: string): Promise<ActionResult<void>> {
    return runAction('removeBonusAttachment', async () => {
        const ctx = await requireInternalOrAdminAction()
        if (!ctx.isAdmin) throw new ExpectedError('Tylko administrator może usunąć załącznik.')
        if (!bonusId) throw new ExpectedError('Brak id premii.')
