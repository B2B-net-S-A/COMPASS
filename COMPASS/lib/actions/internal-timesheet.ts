...
function validateMonth(month: number) {
    if (!Number.isInteger(month) || month < 1 || month > 12) {
        throw new ExpectedError('Miesiąc musi być w zakresie 1–12.')
    }
}
function validateYear(year: number) {
    if (!Number.isInteger(year) || year < 2024 || year > 2100) {
        throw new ExpectedError('Nieprawidłowy rok.')
    }
}
...
export async function addEntry(input: AddEntryInput): Promise<ActionResult<TimesheetEntryRow>> {
    return runAction('addEntry', async () => {
        const ctx = await requireInternalOrAdminAction()
        ...
        if (!input.description?.trim()) {
            throw new ExpectedError('Opis jest wymagany.')
        }
        ...
        if (headerErr || !header) throw new ExpectedError('Timesheet nie istnieje.')
 ...
 */
export async function quickFillMonth(input: QuickFillMonthInput): Promise<ActionResult<QuickFillMonthResult>> {
    return runAction('quickFillMonth', async () => {
        const ctx = await requireInternalOrAdminAction()
        const supabase = createClient()
        ...
        if (header.status !== 'draft') {
            throw new ExpectedError('Można wypełnić tylko timesheet w statusie "draft".')
        }
...
export async function updateEntry(input: UpdateEntryInput): Promise<ActionResult<void>> {
    return runAction('updateEntry', async () => {
        ...
        if (input.description !== undefined) {
            if (!input.description.trim()) throw new ExpectedError('Opis nie może być pusty.')
...
export async function submitTimesheet(timesheetId: string): Promise<ActionResult<void>> {
    return runAction('submitTimesheet', async () => {
        const ctx = await requireInternalOrAdminAction()
        const supabase = createClient()
        const { data: header, error: fetchErr } = await supabase
            .from('timesheets')
        ...
        if (header.status !== 'draft') {
            throw new ExpectedError('Można złożyć tylko timesheet w statusie "draft".')
        }

        const entries = await fetchEntries(timesheetId)
        if (entries.length === 0) {
            throw new ExpectedError('Timesheet musi zawierać przynajmniej jeden wpis.')
        }
...
export async function approveTimesheet(timesheetId: string): Promise<ActionResult<void>> {
    return runAction('approveTimesheet', async () => {
        ...
        if (header.status !== 'submitted') {
            throw new ExpectedError('Można zaakceptować tylko timesheet w statusie "submitted".')
        }
        ...
        if (!ctx.isAdmin) {
            ...
            if (targetProfile?.manager_id !== ctx.userId) {
                throw new ExpectedError('Możesz akceptować timesheety tylko swojego zespołu.')
            }
        }
...
export async function rejectTimesheet(timesheetId: string, reason: string): Promise<ActionResult<void>> {
    return runAction('rejectTimesheet', async () => {
        // Phase 20: timesheet approver = admin OR manager (manager scoped to own team).
        const ctx = await requireTimesheetApproverAction()
        if (!reason?.trim()) throw new ExpectedError('Powód odrzucenia jest wymagany.')
        const admin = createServiceClient()
        ...
        if (fetchErr || !header) throw new ExpectedError('Timesheet nie istnieje.')
        if (header.status !== 'submitted') {
            throw new ExpectedError('Można odrzucić tylko timesheet w statusie "submitted".')
        }
        ...
        if (!ctx.isAdmin) {
            const { data: targetProfile } = await admin
                ...
                .single<{ manager_id: string | null }>()
            if (targetProfile?.manager_id !== ctx.userId) {
                throw new ExpectedError('Możesz odrzucać timesheety tylko swojego zespołu.')
            }
        }
...
export async function unlockTimesheet(timesheetId: string): Promise<ActionResult<void>> {
    return runAction('unlockTimesheet', async () => {
...
async function assertApproverTeamScope(
    ...
    if (target?.manager_id !== ctx.userId) {
        throw new ExpectedError('Możesz edytować timesheety tylko swojego zespołu.')
    }
}
...
async function loadApproverEditableTimesheet(
    ...
    const { data: header, error } = await admin
        ...
        .eq('id', timesheetId)
        .single<Pick<TimesheetHeader, 'id' | 'user_id' | 'year' | 'month' | 'status'>>()
    if (error || !header) throw new ExpectedError('Timesheet nie istnieje.')
...
function resolveOvertimeColumns(
    ...
    if (hours > OVERTIME_OVERRIDE_HOURS_MAX) {
        throw new ExpectedError(`Maksymalnie ${OVERTIME_OVERRIDE_HOURS_MAX}h/dzień (nadgodziny).`)
    }
    ...
    if (trimmed.length > OVERTIME_REASON_MAX_LENGTH) {
        throw new ExpectedError(`Uzasadnienie za długie (max ${OVERTIME_REASON_MAX_LENGTH} znaków).`)
    }
    return {
...
export async function approverAddEntry(input: AddEntryInput): Promise<ActionResult<TimesheetEntryRow>> {
    return runAction('approverAddEntry', async () => {
        const ctx = await requireTimesheetApproverAction()
        if (!/^\d{4}-\d{2}-\d{2}$/.test(input.workDate)) {
            throw new ExpectedError('work_date musi być w formacie YYYY-MM-DD.')
        }
        if (!Number.isFinite(input.hours) || input.hours <= 0 || input.hours > OVERTIME_OVERRIDE_HOURS_MAX) {
            throw new ExpectedError(`Godziny muszą być w zakresie (0, ${OVERTIME_OVERRIDE_HOURS_MAX}].`)
        }
        if (!input.description?.trim()) throw new ExpectedError('Opis jest wymagany.')
...
export async function approverUpdateEntry(input: UpdateEntryInput): Promise<ActionResult<TimesheetEntryRow>> {
    return runAction('approverUpdateEntry', async () => {
        const ctx = await requireTimesheetApproverAction()
        const admin = createServiceClient()
        ...
        if (entry.is_overtime_override && !ctx.isAdmin && !ctx.canLogOvertime) {
            throw new ExpectedError('Ten wpis to nadgodziny — może go edytować administrator lub osoba z uprawnieniem do nadgodzin.')
        }
...
export async function approverDeleteEntry(entryId: string): Promise<ActionResult<void>> {
    return runAction('approverDeleteEntry', async () => {
        const ctx = await requireTimesheetApproverAction()
        const admin = createServiceClient()
        ...
        if (entry.is_overtime_override && !ctx.isAdmin && !ctx.canLogOvertime) {
            throw new ExpectedError('Ten wpis to nadgodziny — może go usunąć administrator lub osoba z uprawnieniem do nadgodzin.')
        }
...
export async function copyPreviousMonthEntries(
    timesheetId: string,
): Promise<ActionResult<CopyPreviousMonthResult>> {
    return runAction('copyPreviousMonthEntries', async () => {
        const ctx = await requireInternalOrAdminAction()
        const supabase = createClient()
        ...
        const { data: header, error: hErr } = await supabase
            ...
            .eq('id', timesheetId)
            .single<Pick<TimesheetHeader, 'id' | 'user_id' | 'year' | 'month' | 'status'>>()
        if (hErr || !header) throw new ExpectedError('Timesheet nie istnieje.')
        if (header.user_id !== ctx.userId && !ctx.isAdmin) {
            throw new ExpectedError('To nie jest Twój timesheet.')
        }
        if (header.status !== 'draft') {
            throw new ExpectedError('Można kopiować opisy tylko do timesheetu w statusie "draft".')
        }
...
export async function ensureTeamTimesheet(
    ...
    year: number,
    month: number,
): Promise<ActionResult<TimesheetWithEntriesAndUser>> {
    return runAction('ensureTeamTimesheet', async () => {
        const ctx = await requireTimesheetApproverAction()
        validateYear(year)
        validateMonth(month)
        const admin = createServiceClient()
...
export async function adminOverrideTimesheetEntry(
    ...
    return runAction('adminOverrideTimesheetEntry', async () => {
        ...
        if (reason.length < OVERTIME_REASON_MIN_LENGTH) {
            throw new ExpectedError(`Uzasadnienie musi mieć co najmniej ${OVERTIME_REASON_MIN_LENGTH} znaki.`)
        }
        ...
        if (fetchErr || !entry) throw new ExpectedError('Wpis nie istnieje.')
 ...
 */
export async function clearOvertimeOverride(entryId: string): Promise<ActionResult<void>> {
    return runAction('clearOvertimeOverride', async () => {
        const ctx = await requireAdminAction()
        const admin = createServiceClient()
        ...
        if (!entry.is_overtime_override) {
            throw new ExpectedError('Ten wpis nie ma aktywnego override — nic do cofnięcia.')
        }
