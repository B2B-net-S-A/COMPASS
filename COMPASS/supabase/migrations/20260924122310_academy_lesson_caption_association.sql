-- A lesson can contain several recordings. Multi-video caption playback uses
-- an explicit association in the versioned lesson snapshot, never list order.
begin;
...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(new.attachments,'[]'::jsonb)) order by value->>'asset_id',value->>'storage_path' loop
    if item ? 'asset_id' then
      ...
      if not found or asset.course_id<>new.course_id or asset.status<>'ready'
