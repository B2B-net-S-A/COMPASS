...
const f=await createAcademyDatabase({runMaterials:true});
...
eq((await sql('select review_status from course_materials where id=$1',[co.id])).rows[0].review_status,'withdrawn');
