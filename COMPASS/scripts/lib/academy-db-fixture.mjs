...
export async function createAcademyDatabase({ materials = false, live = false, staff = false, runMaterials = false, cleanup = false, revocations = false, rollout = false, obligations = false, reviewSubmissions = false, completionGaps = false, administrationControls = false, runsPagination = false, materialReviewIndependence = false, qaRewards = false, invitationTargets = false, enrollmentsPagination = false, m365IdentitiesPagination = false, myRunOverview = false, organizerBudget = false, activationBudget = false, materialProjection = false, beforeAcademyMigrations, beforeQaRewardsMigration, afterBaselineVerification } = {}) {
    if (materialProjection) runMaterials = true;
    if (activationBudget) organizerBudget = true;
    if (organizerBudget) { myRunOverview = true; invitationTargets = true; enrollmentsPagination = true; m365IdentitiesPagination = true; }
    if (myRunOverview) runsPagination = true;
    if (m365IdentitiesPagination) invitationTargets = true;
    if (enrollmentsPagination) runsPagination = true;
    if (invitationTargets) { staff = true; rollout = true; }
    if (qaRewards) { runsPagination = true; materialReviewIndependence = true; }
...
await sql("insert into course_enrollments(user_id,course_id,completed_at,points_awarded,certificate_hash,completed_lessons)values($1,$2,now(),true,'preserved-legacy-hash',$3)",[ids.student,legacy,[oldLesson]]);
    ...
    const suffixes = ['academy_versioned_foundation', ...(materials ? ['academy_materials'] : []), ...(live ? ['academy_live_sessions'] : []), ...(staff ? ['academy_staff_and_legacy_review'] : []), ...(runMaterials ? ['academy_run_materials'] : []), ...(revocations ? ['academy_completion_revocations'] : []), ...(rollout ? ['academy_rollout_gate'] : []), ...(obligations ? ['academy_session_obligations'] : []), ...(cleanup ? ['academy_material_cleanup'] : []), ...(reviewSubmissions ? ['academy_review_submission_token'] : []), ...(completionGaps ? ['academy_roster_progress', 'academy_operations_health', 'academy_attendance_recovery'] : []), ...(administrationControls ? ['academy_archive_controls', 'academy_review_history', 'academy_archive_prerequisite_guard'] : []), ...(runsPagination ? ['academy_runs_pagination'] : []), ...(materialReviewIndependence ? ['academy_material_review_independence'] : []), ...(qaRewards ? ['academy_qa_reward_bounds'] : []), ...(invitationTargets ? ['academy_verified_invitation_targets'] : []), ...(enrollmentsPagination ? ['academy_my_enrollments_page'] : []), ...(m365IdentitiesPagination ? ['academy_m365_identities_page'] : []), ...(myRunOverview ? ['academy_my_run_overview'] : []), ...(organizerBudget ? ['academy_managed_organizer_budget'] : []), ...(activationBudget ? ['academy_activation_invitation_budget'] : []), ...(materialProjection ? ['academy_material_read_projection', 'academy_material_invoker_projection', 'academy_material_revoke_raw_read'] : []), ...(live ? ['academy_session_link_read_guard'] : [])];
    ...
    if (invitationTargets) await db.exec("UPDATE public.academy_rollout_settings SET mode='open'");
