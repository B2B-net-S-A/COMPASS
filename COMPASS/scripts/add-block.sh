#!/usr/bin/env bash
# add-block.sh — pull UI building blocks into Compass safely (no config clobber).
#
#   ./scripts/add-block.sh primitive <name...>     # shadcn/ui atoms  -> components/ui/
#   ./scripts/add-block.sh shadcnblock <slug...>   # shadcnblocks ELITE -> components/blocks/
#
# Why not `npx shadcn add`? The CLI reformats tailwind.config.ts, remaps
# --sidebar -> --sidebar-background (which Compass does NOT define -> breaks the
...
for f in d.get('files',[]):
    out=os.path.join('components/ui', os.path.basename(f['path']))
    open(out,'w').write(f['content']); print('wrote', out)
deps=d.get('dependencies') or []
if deps: print('  npm deps:', ' '.join(deps))
PY
      rm -f "$tmp"
      audit "components/ui/$n.tsx"
    done
  ...
  shadcnblock)
    ...
    if [ "$(date +%Y-%m-%d)" '>' "2026-08-21" ]; then
      echo "⚠ shadcnblocks key may be expired — regenerate at shadcnblocks.com/dashboard/api"
    fi
...
d=json.load(open(sys.argv[1]))
for f in d.get('files',[]):
    out=os.path.join('components/blocks', os.path.basename(f['path']))
    open(out,'w').write(f['content']); print('wrote', out)
deps=d.get('dependencies') or []
if deps: print('  npm deps:', ' '.join(deps))
PY
      rm -f "$tmp"
      audit "components/blocks/$slug.tsx"
    done
