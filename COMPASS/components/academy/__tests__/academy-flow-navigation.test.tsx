...
const run = { id: 'run', courseId: 'course', versionId: 'version', versionNumber: 1, courseTitle: 'Spotkanie testowe', courseSlug: 'live', title: 'Grupa wrześniowa', capacity: 20, status: 'published', confirmedCount: 20, waitlistCount: 1, canManage: false, canPublish: false, sessions: [], myRegistration: { id: 'registration', status: 'waitlisted', enrollmentId: null, completedAt: null } } satisfies AcademyRunDTO
...
describe('learner navigation and personal progress', () => {
    ...
    it('shows waitlist participation even before an enrollment exists', () => {
        render(<AcademyMyLearning enrollments={[]} runs={[run]} now="2026-09-22T10:00:00Z" />)
    ...
    it('never presents zero lesson progress for a live-only course', () => {
        render(<AcademyMyLearning enrollments={[{ ...enrollment, total_lessons: 0, run_id: 'run', course: { ...enrollment.course, delivery_mode: 'live' } }]} runs={[]} now="2026-09-22T10:00:00Z" />)
    ...
    it('preserves a revoked completion as history without an available certificate', () => {
        render(<AcademyMyLearning enrollments={[{ ...enrollment, completed_at: '2026-09-20T10:00:00Z', completion_revoked_at: '2026-09-22T10:00:00Z', completion_revoked_reason: 'Korekta obecności' }]} runs={[]} now="2026-09-22T10:00:00Z" />)
...
it('does not claim there are no registrations when the runs read failed', () => {
    render(<AcademyMyLearning enrollments={[]} runs={undefined} now="2026-09-22T10:00:00Z" />)
