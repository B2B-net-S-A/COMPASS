...
import { CheckCircle2, Loader2, Pencil, Plus, RefreshCw, Search, Video } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from '@/components/ui/dialog'
import { listAcademyOrganizerCandidates, retryAcademyIntegration, saveAcademyOrganizer } from '@/lib/actions/academy-sessions'
import type { AcademyIntegrationIssueDTO, AcademyOrganizerDTO } from '@/lib/types/academy-sessions'
import { SESSION_SELECT_CLASS, sessionDate, sessionTime } from './session-format'

interface Candidate { id: string; fullName: string | null; email: string }
interface Props {
    organizers: AcademyOrganizerDTO[]
    candidates: Candidate[]
    issues: AcademyIntegrationIssueDTO[]
...
export function AcademyIntegrationsPanel({ organizers, candidates, issues, managedTeamsAvailable, reason, rawAttendanceRetentionDays, retentionWarning }: Props) {
    ...
    function retry(jobId: string) {
        ...
        setMessage(null)
        setBusyJob(jobId)
        startTransition(async () => { try { const result = await retryAcademyIntegration(jobId); if (!result.success) { setError(result.error); return } setMessage('Zlecono ponowienie operacji. Odśwież listę, aby sprawdzić wynik.'); router.refresh() } catch { setError('Nie udało się ponowić operacji.') } finally { setBusyJob(null) } })
    }
    return <div className="space-y-6">
        ...
        {error && <p role="alert" className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 text-sm text-destructive">{error}</p>}{message && <p role="status" className="rounded-xl border border-success/20 bg-success/5 p-4 text-sm text-success">{message}</p>}
        ...
        <section className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="space-y-1"><h2 className="text-lg font-semibold">Stan synchronizacji</h2><p className="text-sm text-muted-foreground">Oczekujące i nieudane operacje spotkań oraz obecności.</p></div><Button variant="outline" onClick={() => router.refresh()}><RefreshCw aria-hidden="true" />Odśwież</Button></div><div className="divide-y divide-border rounded-2xl border border-border bg-card">{issues.length === 0 ? <p className="flex items-center gap-2 p-6 text-sm text-muted-foreground"><CheckCircle2 aria-hidden="true" className="size-5 text-success" />Brak operacji wymagających uwagi.</p> : issues.map((job) => <div key={job.id} className="space-y-3 p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="space-y-1"><Link href={`/learning/edycje/${job.runId}`} className="font-medium text-primary underline-offset-4 hover:underline">{job.sessionTitle}</Link><p className="text-sm text-muted-foreground">{JOB_LABEL[job.kind]} · {JOB_STATUS[job.status]}</p><p className="text-xs text-muted-foreground">Próby: {job.attempts} · aktualizacja {sessionDate(job.updatedAt)} {sessionTime(job.updatedAt)}</p>{(job.status === 'retry' || job.status === 'pending') && <p className="text-xs text-muted-foreground">Następna próba: {sessionDate(job.nextAttemptAt)} {sessionTime(job.nextAttemptAt)} (Europe/Warsaw)</p>}</div>{(job.status === 'failed' || job.status === 'retry') && <Button variant="outline" size="sm" disabled={isPending || busyJob !== null} onClick={() => retry(job.id)}>{busyJob === job.id ? <Loader2 aria-hidden="true" className="animate-spin" /> : <RefreshCw aria-hidden="true" />}Ponów</Button>}</div>{job.lastError && <p className="break-words rounded-lg bg-destructive/5 p-3 text-sm text-destructive">{job.lastError}</p>}</div>)}</div></section>
