...
export function AcademyAttendancePanel({ participants, sessions, userId, readOnly = false }: { participants: AcademyRunParticipantDTO[]; sessions: AcademySessionDTO[]; userId: string; readOnly?: boolean }) {
    ...
    function save(event: React.FormEvent<HTMLFormElement>) {
        ...
        startTransition(async () => { try {
            const result = await recordAcademyAttendance({ sessionId: session.id, enrollmentId, status: data.get('status') === 'present' ? 'present' : 'insufficient', attendedSeconds: minutes ? Math.round(Number(minutes) * 60) : undefined, note: String(data.get('note') ?? '').trim() })
