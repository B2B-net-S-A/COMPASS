...
export default async function AcademyCalendarPage({ searchParams }: { searchParams: { course?: string; month?: string; mine?: string; page?: string } }) {
    ...
    // Include edge sessions whose Warsaw date differs from the UTC date.
    const start = new Date(`${month}-01T00:00:00Z`).getTime() - 2 * 86400000
    const end = new Date(`${shiftCalendarMonth(month, 1)}-01T00:00:00Z`).getTime() + 2 * 86400000
    const [access, runs] = await Promise.all([getAcademyAccess(), listAcademyRunPage({ courseId, scope: mine ? 'my_calendar' : 'calendar', page, pageSize: 50, windowStart: new Date(start).toISOString(), windowEnd: new Date(end).toISOString() })])
