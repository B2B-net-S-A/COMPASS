...
describe("GET /api/internal/workdays", () => {
  it("zwraca dni robocze przy poprawnym sekrecie", async () => {
    const res = await call(
      "http://x/api/internal/workdays?from=2026-08&to=2026-08",
      {
  ...
  it("przyjmuje granulację tygodniową dla Power Callingu", async () => {
    // Power Calling raportuje TYDZIEŃ ISO; przybliżanie go z miesięcznej
    // średniej byłoby zgadywaniem, czyli defektem, który tu likwidujemy.
    const res = await call(
      "http://x/api/internal/workdays?from=2026-08-24&to=2026-08-24&bucket=week",
      { authorization: `Bearer ${SECRET}` },
    );
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("waliduje zakres i granulację", async () => {
    const bad = await call(
      "http://x/api/internal/workdays?from=sierpien&to=2026-08",
      {
        authorization: `Bearer ${SECRET}`,
      },
    );
    ...
    const reversed = await call(
      "http://x/api/internal/workdays?from=2026-09&to=2026-08",
      {
        authorization: `Bearer ${SECRET}`,
      },
    );
    ...
    const tooWide = await call(
      "http://x/api/internal/workdays?from=2020-01&to=2026-08",
      {
        authorization: `Bearer ${SECRET}`,
      },
    );
    ...
    // inaczej literówka dałaby liczby miesięczne pod etykietą tygodnia.
    const badBucket = await call(
      "http://x/api/internal/workdays?from=2026-08&to=2026-08&bucket=kwartal",
      { authorization: `Bearer ${SECRET}` },
    );
