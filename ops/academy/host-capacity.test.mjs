...
test('capacity projection accepts only complete numeric counters',()=>{
 ...
 assert.equal(parseCapacity(output).composeMajor,2);assert.equal(parseCapacity(output).composeMinor,39);
 assert.equal(parseCapacity(output).unitShowExitCode,4);
 assert.throws(()=>parseCapacity(output.replace('unitShowExitCode=4','unitShowExitCode=256')));
...
test('Compose diagnostics report versions without imposing the installer compatibility policy',()=>{
 for(const [major,minor]of [[5,0],[3,7],[0,0]]){
  const report=parseCapacity(output.replace('composeMajor=2',`composeMajor=${major}`).replace('composeMinor=39',`composeMinor=${minor}`));
  assert.equal(report.composeMajor,major);assert.equal(report.composeMinor,minor);
 }
 for(const invalid of [output.replace('composeMajor=2','composeMajor=1000'),output.replace('composeMinor=39','composeMinor=-1'),output.replace('composeMajor=2','composeMajor=0'),output.replace('composeMinor=39\n','')])assert.throws(()=>parseCapacity(invalid));
});
test('fixed host-key verified SSH command reads only capacity and never emits key or target',async()=>{
 ...
 const report=await collectHostCapacity({env,run:async(binary,args,options)=>{
  ...
  const command=args.at(-1);assert.match(command,/\/proc\/meminfo/);assert.match(command,/df -Pk/);assert(!/sudo |printenv|\.env|curl |wget /.test(command));
  assert.equal((command.match(/docker --host unix:\/\/\/var\/run\/docker\.sock compose version --short/g)??[]).length,1);
  const otherCommands=command.split('\n').filter(line=>!line.trim().startsWith('#')).join('\n')
   .replace('command -v docker >/dev/null 2>&1','').replace('docker --host unix:///var/run/docker.sock compose version --short','');
  assert(!/\bdocker\b/.test(otherCommands));
  assert.match(command,/systemctl show compass-academy-materials\.service --property=LoadState,ActiveState,UnitFileState >\/dev\/null 2>&1/);
  assert(!/systemctl (?:start|stop|enable|disable|restart|daemon-reload)/.test(command));
