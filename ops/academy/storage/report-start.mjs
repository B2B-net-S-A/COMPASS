...
export function projectStart(mode, status, log, migrationCount, orderingExceptions = [], archivedBootstraps = []) {
    ...
    const started = new Set(migrations.map(item=>item[1])).size;
    const complete = mode !== 'historical-replay' || (skipped.length===0 && started===migrationCount && migrationCount>0);
    ...
    return {
        mode, outcome: passed ? 'passed' : 'failed', exitCode: status,
        startedMigrationFiles: started, appliedMigrationFiles: passed ? started : null, skippedMigrationFiles: skipped,
        historicalMigrationFiles: mode === 'historical-replay' ? migrationCount - archivedBootstraps.length : null,
        replayFilesExpected: mode === 'historical-replay' ? migrationCount : null,
        archivedBootstraps: mode === 'historical-replay' ? archivedBootstraps : [],
        ...
        originalHistoryUnchanged: mode === 'historical-replay' ? orderingExceptions.length === 0 && archivedBootstraps.length === 0 : null,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
    ...
    const archivedBootstraps = manifest.filter(row => row.sourceKind === 'archived_bootstrap').map(row => ({ sourceRelativePath: row.sourceRelativePath, sha256: row.sha256 }));
