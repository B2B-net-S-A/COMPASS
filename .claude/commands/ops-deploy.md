...
RESPONSE=$(curl -sf -X GET \
  ...
  "https://coolify-compass.dynaminds.pl/api/v1/deploy?uuid=$UUID&force=$FORCE")
...
for i in {1..20}; do
  STATUS=$(curl -sf -H "Authorization: Bearer $TOKEN" -H "Accept: application/json" \
    "https://coolify-compass.dynaminds.pl/api/v1/deployments/$DEPLOY_UUID" | jq -r .status)
  echo "[$i/20] status=$STATUS"
  case "$STATUS" in
    finished) break ;;
    failed) echo "DEPLOY FAILED — fetch logs:"; curl -sf -H "Authorization: Bearer $TOKEN" "https://coolify-compass.dynaminds.pl/api/v1/deployments/$DEPLOY_UUID" | jq -r .logs | tail -50; exit 1 ;;
  esac
  sleep 30
done
```
...
for i in {1..12}; do
  HEALTH=$(curl -fsSL "https://compass.dynaminds.pl/api/health" 2>/dev/null)
