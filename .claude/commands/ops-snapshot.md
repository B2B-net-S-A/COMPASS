---
description: Snapshot compass state (health + KPIs + queues) via /api/admin/snapshot
allowed-tools: Bash(source:*), Bash(curl:*), Bash(jq:*), Bash(git rev-parse:*), Bash(echo:*)
---

Pobierz pełny snapshot stanu **compass** w jednym requeście (zamiast 5 round-tripów).

**Step 1 — fetch:**

```bash
source ~/.claude/secrets/dynaminds-tokens.env
curl -fsSL -H "X-Snapshot-Token: $COMPASS_SNAPSHOT_TOKEN" "https://compass.dynaminds.pl/api/admin/snapshot" | jq .
```
...
**Error handling:**
- 404 → `/api/admin/snapshot` jeszcze nie zdeployowany (Phase 2 pending dla compass). Powiedz to userowi.
- 401 → token problem. Verify: `echo "${COMPASS_SNAPSHOT_TOKEN}"` (powinno zwrócić wartość, nie pusty string). Jeśli pusto — `source ~/.claude/secrets/dynaminds-tokens.env` i retry.
