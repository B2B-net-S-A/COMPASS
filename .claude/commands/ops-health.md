---
description: Quick health check for compass via /api/health
allowed-tools: Bash(curl:*), Bash(jq:*), Bash(git rev-parse:*)
---

Szybki health check **compass** — wersja, status DB, deploy time.

```bash
SHORT_SHA=$(git rev-parse --short=7 HEAD 2>/dev/null || echo "<not-in-git>")
echo "Local HEAD: $SHORT_SHA"
curl -fsSL "https://compass.dynaminds.pl/api/health" | jq .
```
...
**Jeśli non-200:**
- 503 → status="unhealthy", at least one check failed. Sugeruj `/ops-logs` dla app service.
- 502 → backend container down lub unstable. Sugeruj `/ops-logs` + Coolify panel check.
- timeout → DNS/network problem albo Cloudflare incident. Test: `curl -I https://https://compass.dynaminds.pl` (gołe headers, sprawdź czy CF zwraca anything).
