...
def nexus_application(applications):
    matches = [app for app in applications
               if repository_name(app.get("git_repository")) == "artur-t-96/nexus"
               and app.get("git_branch") == "main"]
    if len(matches) != 1:
        raise ValueError("Expected exactly one NEXUS application on main")
    return matches[0]
...
def main():
    ...
    compass_id = os.environ["COOLIFY_APP_UUID"]
    ...
    compass = api("GET", "/applications/" + compass_id)
    if repository_name(compass.get("git_repository")) != "artur-t-96/compass" or compass.get("git_branch") != "main":
        raise ValueError("Configured COMPASS application does not match the production repository")
    nexus = nexus_application(api("GET", "/applications"))
    nexus_id = nexus["uuid"]

    def envs(app_id):
        return {row["key"]: row for row in api("GET", "/applications/" + app_id + "/envs")
    ...
    changes = (["COMPASS:AVAILABILITY_EXPORT_SECRET", "NEXUS:COMPASS_AVAILABILITY_SECRET", "NEXUS:COMPASS_AVAILABILITY_URL"]
               if stage == "export" else ["NEXUS:COMPASS_AVAILABILITY_ENABLED", "NEXUS:RECRUITMENT_ALLOCATION_ENABLED"])
    print(json.dumps({"dry_run": dry_run == "true", "stage": stage, "settings": changes,
                      "source_configured": bool(source), "consumer_configured": bool(target)}))
    if dry_run == "true":
        return

    def put(app_id, existing, key, value):
        api("PATCH" if key in existing else "POST", "/applications/" + app_id + "/envs", {
            "key": key, "value": value, "is_buildtime": False, "is_runtime": True, "is_preview": False,
        })

    if stage == "export":
        if source and target and source != target:
            raise ValueError("Existing availability credentials disagree; refusing rotation")
        secret = source or target or secrets.token_urlsafe(48)
        if len(secret) < 32 or "*" in secret:
            raise ValueError("Vault did not return a usable credential; refusing replacement")
        put(compass_id, compass_env, source_key, secret)
        put(nexus_id, nexus_env, target_key, secret)
        put(nexus_id, nexus_env, "COMPASS_AVAILABILITY_URL", "https://compass.dynaminds.pl/api/internal/availability")
        restart_id = compass_id
    else:
        if not source or not target:
            raise ValueError("Configure the export connection before enabling the consumer")
        if source != target:
    ...
    print(json.dumps({"configured": True, "deployment_ids": [row.get("deployment_uuid") for row in (result or {}).get("deployments", [])]}))
