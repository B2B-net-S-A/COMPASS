# Historia faz COMPASS (fazy 17–54, maj–sierpień 2026)

> **To jest archiwum, nie instrukcja.** Plik powstał przy audycie 2026-08-25 (pozycja C9),
> gdy `CLAUDE.md` urósł do 156 KB i był w całości ładowany do kontekstu każdej sesji.
> Zostały tu opisy **co i kiedy zostało zrobione**. Reguły, których złamanie dziś coś psuje,
> przeniesiono do [`CLAUDE.md`](../CLAUDE.md) — jeśli szukasz „jak nie zepsuć", zacznij tam.
>
> **Zasada czytania:** ten plik opisuje stan z dnia wdrożenia danej fazy. Późniejsze fazy
> (i audyt 2026-08) niektóre z tych rozstrzygnięć unieważniły. Miejsca, w których wiadomo,
> że opis rozjechał się z kodem, mają wstawkę „⚠ Nieaktualne". Wszędzie indziej **kod
> i baza są źródłem prawdy, nie ten plik**.
>
> Fazy nie są w kolejności numerycznej — zachowano kolejność z oryginalnego `CLAUDE.md`
> (dopisywano na końcu, a numery faz nadawano niezależnie od dat wdrożenia).

---

## Phase 17 — Smart Work Clock: crony (2026-05-08)

> ⚠ **Nieaktualne.** Moduł Work Clock jest usuwany w audycie 2026-08 (pozycja C3): UI, trasy
> `app/api/clock/*` i crony `clock-*` znikają z kodu. Tabele `work_clock_*` zostają w bazie,
> bo czyta z nich eksport RODO. Zadania `clock-daily-cutoff` / `clock-idle-reaper` /
> `clock-daily-summary` / `clock-route-retention` trzeba **wyłączyć w Coolify**, inaczej
> tikają w nieistniejący endpoint. Opis niżej to zapis stanu z maja.

Po merge PR #54 dodano 2 zadania w panelu Coolify (Resources → compass → Schedules):

| Nazwa | Schedule | Komenda |
|---|---|---|
| `clock-daily-cutoff` | `0 4 * * *` | `curl -fsS -H "Authorization: Bearer $CRON_SECRET" ".../api/cron/clock-daily-cutoff"` |
| `clock-idle-reaper` | `*/15 * * * *` | `curl -fsS -H "Authorization: Bearer $CRON_SECRET" ".../api/cron/clock-idle-reaper"` |

**Co robiły:**
- `clock-daily-cutoff` — zamykał sesje (`work_clock_sessions` z `ended_at IS NULL`) starsze niż 16 h, reason `daily_cutoff`.
- `clock-idle-reaper` — zamykał sesje z `last_heartbeat < NOW() - 60 min` (network drop, uśpiony laptop bez `sendBeacon`), reason `idle_timeout`.

**Auth:** `CRON_SECRET`; preferowany nagłówek Bearer (sekret nie loguje się w CF/proxy/Sentry),
legacy `?secret=` działał z ostrzeżeniem.
...
> ⚠ **Nieaktualne (audyt 2026-08-25, C3/C9).** Opisany niżej hub pod `/internal/kontraktorzy`
> **nie istnieje jako ekran** — `app/(protected)/internal/kontraktorzy/page.tsx` to samo
> `redirect()` (na `/internal/people?tab=kontraktorzy`, a przy włączonej fladze Consultant
> Success na `/internal/people/success/consultants`). Panele `KontraktorzyHub` + `panels/*`
> (~840 linii) są nieosiągalne z żadnego page/layout/route. Bieżącym ekranem jest hub
> **People Ops** (`/internal/people`). `/internal/onboarding` też jest tylko przekierowaniem —
...
blokada miałaby dziury tam, gdzie akurat nie potrzebowaliśmy roli (`/profile`, `/documents`
ten SELECT pomijały). Koszt: jeden lookup po PK. Wszystkie redirecty zależne od roli są
path-scoped, więc szerszy fetch nie zmienia niczego poza samą blokadą.

### DWIE REGUŁY filtrowania list ludzi — łatwo ujednolicić w złą stronę

> ⚠ **Reguła nadal obowiązuje, ale jej kanoniczny zapis jest w [`CLAUDE.md`](../CLAUDE.md),
> a nazwy funkcji się zmieniły.** Audyt 2026-08 (B4) wystawił dwie jawne funkcje w
> `lib/hr/employment-window.ts`: `activeRoster()` (listy „tu i teraz") i `employedInMonth()` /
> `filterEmployedInMonth()` (raporty miesięczne). Nowy kod woła je, a nie wkleja
> `.neq('employment_status','exited')` inline. Poniżej oryginalne sformułowanie z lipca.

Dodając zapytanie do `profiles` zwracające wielu ludzi, wybierz świadomie:

- **Listy „tu i teraz"** (dropdowny przypisania, rozsyłki powiadomień, crony mailowe,
  katalogi ludzi) → `.neq('employment_status', 'exited')`. Tak robią już payroll, stawki,
...
Naprawione w #290 (11 miejsc): adresaci wiadomości w module komunikatora (jedyny **żywy**
wyciek, reszta nie strzelała przez zbieg okoliczności — sam moduł usunięto w audycie
2026-08, pozycja C4), crony `timesheet-reminder` i `clock-daily-summary`, `payroll-export`,
`listManagerCandidates`, dropdowny TCM (`listTcmProfiles`, `loadTcmOptions`, ticket→zadanie
w inboxie, planner), `listHandlers` skrzynki, powiadomienia przy publikacji newsa.
...
> ⚠ **Częściowo nieaktualne.** Ta faza (i Faza 49) to trzecia i czwarta łatka punktowa na
> ten sam problem; audyt 2026-08 (B1) rozwiązał go u przyczyny kontraktem `runAction` /
> `ExpectedError` — wyjątek jest teraz **zwracany jako dane**, więc komunikat dociera do
> użytkownika bez maskowania i bez łatki per pole. Sam nawyk „limit z CHECK-a powtórz
> w walidacji" zostaje aktualny (lepszy komunikat, mniej round-tripów), ale nie jest już
> jedyną obroną przed bełkotem. Kanoniczny opis kontraktu: [`CLAUDE.md`](../CLAUDE.md).
...
**Ops po deploy — planowane tak, wykonane inaczej:**

> ⚠ **Nieaktualne.** Pierwotny plan mówił „dodać w Coolify zadanie `legal-monitor-alerts`
> (`0 8 * * *`)". **Nigdy go nie dodano** — przez dwa tygodnie nie wyszedł ani jeden alert
> (odkryte 2026-08-24). Harmonogramem tej trasy jest **GH Actions**
> (`.github/workflows/cron-legal-monitor-alerts.yml`, `23 8 * * *` — pełna godzina trafia
> w szczyt GH i pierwsza próba `0 8` nie odpaliła w ogóle). Powód i pełna diagnoza:
> sekcja Phase 54. **Nie dodawaj bliźniaczego zadania w Coolify** — tygodniowy digest
> nie ma stempla dedupu, więc dwa schedulery = dwa maile w poniedziałki.
