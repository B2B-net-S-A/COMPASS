...
**Własna frekwencja uczestnika — implementacja gotowa do odbioru:** widok edycji pokazuje stan i potwierdzony czas własnej obecności, próg z przypisanej wersji oraz spełnienie obowiązku każdej sesji. Brak danych pozostaje jawnym brakiem potwierdzenia; odwołana sesja nie zalicza obowiązku, a po zastępstwie liczy się nowa sesja. Uczestnik widzi konkretne brakujące lekcje, quiz i spotkania, z linkami zachowującymi jego zapis. Odczyt korzysta z istniejącego RPC edycji, sprawdza `auth.uid()`, nie udostępnia notatek prowadzącego i nie zmienia ukończeń. Lokalny dowód: 86 asercji SQL (w tym IDOR, prywatne notatki, przypisana wersja, zastępstwa i brak mutacji odczytu) oraz 14 testów UI. Pozostaje hosted CI i rzeczywisty odbiór na koncie uczestnika po wdrożeniu; osobny follow-up implementacyjny nie jest już potrzebny.
