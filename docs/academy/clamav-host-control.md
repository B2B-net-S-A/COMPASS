...
Aktywacja jest oddzielnym krokiem po wdrożeniu migracji/aplikacji, utworzeniu sieci normalną trasą deploy, odebraniu VERSION i skanu oraz potwierdzeniu bezpiecznej retencji dla sync. Nie ma jednostki cleanup. `ACADEMY_MATERIAL_CLEANUP_ENABLED` pozostaje false, a `ACADEMY_ATTENDANCE_RETENTION_DAYS` nie należy ustawiać bez decyzji retencyjnej. Po gotowości operator może włączyć wyłącznie trzy timery:

```bash
sudo systemctl enable --now compass-academy-materials.timer compass-academy-sync.timer compass-academy-clamav-update.timer
