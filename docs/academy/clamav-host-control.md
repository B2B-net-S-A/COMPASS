...
Host odnajduje dokładnie jeden działający kontener `service=app` o prefiksie zasobu Coolify `app-w136dv828ofipvjfnxrqi643-`, ponownie weryfikuje jego tożsamość i wykonuje `docker exec --interactive <pełne ID> node --input-type=module - <worker>`; kod jest przekazany stdin. Brak albo wiele dopasowań blokuje wykonanie. Coolify zmienia nazwę przy każdym deployu; nie używać stałej nazwy `compass-app`. Sekret `CRON_SECRET` odczytuje dopiero Node wewnątrz kontenera, wywołując allowlistowany `http://127.0.0.1:10000/api/cron/academy-materials` albo `academy-sync`. Nie ma sekretu na hoście, w argumentach, jednostce czy logu. Log zawiera tylko wynik, worker i HTTP status; błędy mają stałe identyfikatory. Docker exec dziedziczy konfigurację środowiska istniejącego kontenera. [Dokumentacja Docker exec](https://docs.docker.com/reference/cli/docker/container/exec/).
...
## Routing publiczny po dołączeniu skanera

Aplikacja ma jawny label `traefik.docker.network=w136dv828ofipvjfnxrqi643`, wskazujący potwierdzoną sieć wspólną z proxy. Sieć `compass-academy-private` pozostaje wewnętrzna; proxy nie może do niej dołączyć. Lokalny healthcheck i gotowość skanera nie zastępują publicznego `/api/health` z oczekiwanym SHA. Diagnostyka `proxy-readiness.mjs` odczytuje wyłącznie ograniczone metadane sieci i health, bez środowiska ani logów kontenerów.
