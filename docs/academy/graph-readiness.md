...
Workflow `.github/workflows/academy-graph-readiness.yml` sprawdza trzy role już używane przez adapter Academy. Nie zmienia istniejącego raportu Coolify, nie otwiera procesu nadawania zgód i nie wykonuje operacji na skrzynkach ani spotkaniach. Uruchamia się na zaufanym hosted runnerze dla wskazanej gałęzi źródłowego repozytorium; lokalne uruchomienie lub PR z forka nie otrzyma dostępu przez helper.
