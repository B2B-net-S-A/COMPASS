...
Pierwszy odczyt CI potwierdził brak ustawień skanera i dwóch schedulerów; szczegóły i data znajdują się w runbooku inventory. Minimalny następny krok: po przeglądzie i push uruchomić rozszerzony [odczyt konfiguracji przez istniejący dostęp CI](./readiness-inventory.md). Ustali on dostępne metadane aplikacji, obecność ustawień i stan znanych schedulerów bez ich komend lub wartości sekretów. Osobny krok tego workflow może odczytać RAM, CPU i dysk przez istniejące poświadczenia SSH z przypiętym kluczem hosta. Limity usług, szczytowe użycie i zapas przy buildzie trzeba nadal odczytać w istniejącym monitoringu lub panelu; pojedynczy pomiar nie daje gwarancji pojemności. Jeżeli obecny host nie ma niezależnego zapasu 4 GiB plus miejsca na proces aktualizacji, wybrać osobny węzeł. Dopiero po tym przygotować prywatną usługę skanera z przypiętym obrazem i wymaganymi katalogami. Ten dokument nie przyznaje uprawnień do zakupu zasobów ani zmiany sieci produkcyjnej.
